...
import BucketsModal from "../components/BucketsModal";

function Orcamento({ showToast }) {
  const { data } = useData();
  ...
  const itens = [
    {
      label: "Necessidades",
      sublabel: "50%",
      gasto: buckets.necessidades,
      limite: limites.necessidades,
      cor: "#3b82f6",
    },
    {
      label: "Desejos",
      sublabel: "30%",
      gasto: buckets.desejos,
      limite: limites.desejos,
      cor: "#8b5cf6",
    },
    {
      label: "Investimentos",
      sublabel: "20%",
      gasto: buckets.investimentos,
      limite: limites.investimentos,
      cor: "#10b981",
    },
  ];
            ...
            <div key={item.label} className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span style={{ color: over ? "#ef4444" : "#10b981" }}>
                  {over
                    ? `Acima em ${fmtBRL(item.gasto - item.limite)}`
                    : `${fmtBRL(item.limite - item.gasto)} restante`}
                </span>
              </div>
              <div className="h-3 bg-gray-700 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${pct}%`,
                    background: over ? "#ef4444" : item.cor,
                  }}
                />
              </div>
      ...
      <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700/50">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Quanto falta para fechar no positivo
            </p>
          </div>
          <div className="text-right">
            <p
              className={`text-2xl font-bold ${saldo >= 0 ? "text-emerald-400" : "text-red-400"}`}
            >
        ...
        <div className="h-3 bg-gray-700 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width:
                receitas > 0
                  ? `${Math.min((despesas / receitas) * 100, 100)}%`
                  : "0%",
            }}
          />
        </div>
        <div className="flex justify-between text-xs text-gray-500 mt-2">
          ...
          <span className={saldo >= 0 ? "text-emerald-400" : "text-red-400"}>
            {saldo >= 0
              ? `${fmtBRL(saldo)} sobrando`
              : `${fmtBRL(Math.abs(saldo))} a descoberto`}
          </span>
        </div>
      </div>
      {showBuckets && (
